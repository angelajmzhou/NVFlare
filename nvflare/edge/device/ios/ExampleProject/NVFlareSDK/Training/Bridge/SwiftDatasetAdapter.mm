...
SwiftDatasetAdapter::SwiftDatasetAdapter(void *swiftObject)
  ...
  NSLog(@"SwiftDatasetAdapter constructor: created at %p with Swift object %p",
        this, swiftObject);
}
...
SwiftDatasetAdapter::~SwiftDatasetAdapter() {
  ...
  if (!isDestroyed.compare_exchange_strong(expected, true)) {
    ...
    NSLog(@"SwiftDatasetAdapter: Already destroyed by another thread! "
          @"(expected=%s)",
          expected ? "true" : "false");
    return;
  }

  NSLog(@"SwiftDatasetAdapter: Successfully acquired destruction lock, "
        @"proceeding with cleanup");

  if (swiftObjectPtr) {
    NSLog(@"SwiftDatasetAdapter: About to release retained Swift object at %p",
          swiftObjectPtr);
    ...
    @try {
      ...
      NSLog(@"SwiftDatasetAdapter: Calling CFBridgingRelease on %p",
            ptrToRelease);
      CFBridgingRelease(ptrToRelease);
      NSLog(@"SwiftDatasetAdapter: CFBridgingRelease completed successfully");

    } @catch (NSException *exception) {
      NSLog(@"SwiftDatasetAdapter: Exception during CFBridgingRelease: %@",
            exception);
    }
...
std::optional<SwiftDatasetAdapter::BatchType>
SwiftDatasetAdapter::getBatch(size_t batchSize) {
  ...
  @autoreleasepool {
    ...
    if (![nvflareDataset respondsToSelector:getNextBatchSelector]) {
      NSLog(@"SwiftDatasetAdapter::getBatch() object does not respond to "
            @"getNextBatchWithBatchSize:");
      return std::nullopt;
    }
    ...
    id nvflareBatch = getNextBatchFunc(nvflareDataset, getNextBatchSelector,
                                       (NSInteger)batchSize);
    if (!nvflareBatch) {
    ...
    if (![nvflareBatch respondsToSelector:getInputSelector] ||
      ...
      NSLog(@"SwiftDatasetAdapter::getBatch() batch does not respond to "
            @"getInput/getLabel");
      return std::nullopt;
    }

    id inputData =
        ((id(*)(id, SEL))objc_msgSend)(nvflareBatch, getInputSelector);
    id labelData =
        ((id(*)(id, SEL))objc_msgSend)(nvflareBatch, getLabelSelector);

    if (!inputData || !labelData) {
      NSLog(@"SwiftDatasetAdapter::getBatch() null input or label data from "
            @"batch");
      return std::nullopt;
    }
    ...
    if (batchSizeActual == 0 || inputVec.size() % batchSizeActual != 0) {
      NSLog(@"SwiftDatasetAdapter::getBatch() input size %zu not evenly "
            @"divisible by batch size %zu",
            inputVec.size(), batchSizeActual);
      return std::nullopt;
    }

    // Determine input shape
    std::vector<int32_t> inputShape;
    inputShape.push_back(static_cast<int32_t>(batchSizeActual));

    SEL dimSel = sel_registerName("getInputDimensions");
    bool dimsFound = false;

    if ([nvflareDataset respondsToSelector:dimSel]) {
      typedef NSArray *(*DimFunc)(id, SEL);
      DimFunc dimFunc = (DimFunc)objc_msgSend;
      NSArray *dims = dimFunc(nvflareDataset, dimSel);
      if (dims && dims.count > 0) {
        for (NSNumber *d in dims) {
          inputShape.push_back(d.intValue);
        }
        dimsFound = true;
      }
    }
    ...
    if (!dimsFound) {
      // Fallback to flattened 2D tensor
      size_t inputDim = inputVec.size() / batchSizeActual;
      inputShape.push_back(static_cast<int32_t>(inputDim));
    }

    // Create tensors
    auto inputTensor = make_tensor_ptr<float>(inputShape, std::move(inputVec));

    auto labelTensor = make_tensor_ptr<int64_t>(
        {static_cast<int>(batchSizeActual)}, std::move(labelVec));

    NSLog(@"SwiftDatasetAdapter::getBatch() returning batch with %zu samples",
          batchSizeActual);
...
void SwiftDatasetAdapter::setShuffle(bool shuffle) {
  NSLog(@"SwiftDatasetAdapter::setShuffle() called with %s",
        shuffle ? "true" : "false");
  ...
  NSLog(@"SwiftDatasetAdapter::setShuffle() NVFlareDataset protocol doesn't "
        @"support setShuffle - no-op");
}
...
size_t SwiftDatasetAdapter::size() const {
  ...
  if (!swiftObjectPtr) {
    NSLog(
        @"SwiftDatasetAdapter::size() NVFlareDataset object pointer is null!");
    return 0;
  }

  NSLog(@"SwiftDatasetAdapter::size() calling NVFlareDataset object at %p",
        swiftObjectPtr);
  ...
  @autoreleasepool {
    ...
    if (![nvflareDataset respondsToSelector:sizeSelector]) {
      NSLog(@"SwiftDatasetAdapter::size() object does not respond to size "
            @"selector");
      return 0;
    }
...
size_t SwiftDatasetAdapter::inputDim() const {
  ...
  @autoreleasepool {
    ...
    if ([nvflareDataset respondsToSelector:getInputDimensionsSelector]) {
      ...
      GetInputDimensionsFunc getInputDimensionsFunc =
          (GetInputDimensionsFunc)objc_msgSend;
      NSArray *dimensions =
          getInputDimensionsFunc(nvflareDataset, getInputDimensionsSelector);
      if (dimensions && dimensions.count > 0) {
        ...
        NSLog(
            @"SwiftDatasetAdapter::inputDim() empty or null dimensions array");
        return 0;
      }
    } else {
      NSLog(@"SwiftDatasetAdapter::inputDim() ERROR: object does not respond "
            @"to getInputDimensions - this is required!");
      throw std::runtime_error("Dataset must implement getInputDimensions "
                               "method - cannot determine input dimensions");
    }
  }
}
...
size_t SwiftDatasetAdapter::labelDim() const {
  ...
  @autoreleasepool {
    ...
    if ([nvflareDataset respondsToSelector:getOutputDimensionsSelector]) {
      ...
      GetOutputDimensionsFunc getOutputDimensionsFunc =
          (GetOutputDimensionsFunc)objc_msgSend;
      NSArray *dimensions =
          getOutputDimensionsFunc(nvflareDataset, getOutputDimensionsSelector);
      if (dimensions && dimensions.count > 0) {
        NSInteger firstDim = [[dimensions firstObject] integerValue];
        ...
        NSLog(
            @"SwiftDatasetAdapter::labelDim() empty or null dimensions array");
        return 1;
      }
    } else {
      NSLog(@"SwiftDatasetAdapter::labelDim() ERROR: object does not respond "
            @"to getOutputDimensions - this is required!");
      throw std::runtime_error("Dataset must implement getOutputDimensions "
                               "method - cannot determine label dimensions");
    }
  }
}
